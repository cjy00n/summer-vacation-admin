...
const TermsOfUsePage = () => {
  ...
  useEffect(() => {
    if (termsOfUses) {
      setCurrentTermsOfUse("약관 V" + termsOfUses[0].version.toFixed(1));
    }
  }, [isSuccess, termsOfUses]);

  return (
    <div>
      <MainTitle title="이용약관" />
      <SubTitle title="약관 선택" />
      <div className="flex items-center mb-6">
        {termsOfUses && (
          <CustomDropDown
            values={termsOfUses.map(
              (item) => "약관 V" + item.version.toFixed(1)
            )}
            ...
            <tbody>
              {termsOfUses.map((use) => (
                <tr key={"faq-category" + use.id}>
                  ...
                  <td className="">
                    <div className="mx-auto">{"약관 V" + use.version}</div>
                  </td>
                  <td>
