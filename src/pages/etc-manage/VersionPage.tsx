...
const VersionPage = () => {
  ...
  useEffect(() => {
    if (termsOfUses) {
      setCurrentTermsOfUse("공지 V" + termsOfUses[0].version.toFixed(1));
    }
  }, [isSuccess, termsOfUses]);

  return (
    <div>
      <MainTitle title="버전 관리" />
      <SubTitle title="버전 공지 선택" />
      <div className="flex items-center mb-6">
        {termsOfUses && (
          <CustomDropDown
            values={termsOfUses.map(
              (item) => "공지 V" + item.version.toFixed(1)
            )}
